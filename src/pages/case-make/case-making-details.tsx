...
const CaseMakingDetails = () => {
  ...
  const handleLinkSubmission = async () => {
    if (!user || !link || !id) {
      return ToastError({ message: "Unauthorized to submit case-making link." });
    }
    ...
    return ToastSuccess({ message: "Successfully submitted case-making link." });
  };
          ...
          <>
            {!user?.roles.includes("Software Subject Coordinator") && (
              <Card className="w-full flex flex-col flex-1">
                <CardHeader>
                  <CardTitle>Case-Making Submission</CardTitle>
                  <CardDescription>
                    Submit case-making's OneDrive link
                  </CardDescription>
                </CardHeader>
...
export default CaseMakingDetails;